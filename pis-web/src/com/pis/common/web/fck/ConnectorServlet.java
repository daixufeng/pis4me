...
import com.pis.common.util.FileNameUtils;
...
//		UtilsFile.checkDirAndCreate(defaultUserFilesDir);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!RequestCycleHandler.isEnabledForFileBrowsing(request))
			xr = new PisXmlResponse(PisXmlResponse.EN_ERROR, Messages.NOT_AUTHORIZED_FOR_BROWSING);
		else if (!CommandHandler.isValidForGet(commandStr))
			xr = new PisXmlResponse(PisXmlResponse.EN_ERROR, Messages.INVALID_COMMAND);
		else if (typeStr != null && !ResourceTypeHandler.isValid(typeStr))
			xr = new PisXmlResponse(PisXmlResponse.EN_ERROR, Messages.INVALID_TYPE);
		else if (!UtilsFile.isValidPath(currentFolderStr))
			xr = new PisXmlResponse(PisXmlResponse.EN_ERROR, Messages.INVALID_CURRENT_FOLDER);
		else {
...
//			String typeDirPath = getServletContext().getRealPath(typePath);
			String typeDirPath =  ConnectorHandler.getUserFilesPath(request);
			...
			if (!currentDir.exists())
				xr = new PisXmlResponse(PisXmlResponse.EN_INVALID_FOLDER_NAME);
			else {

				xr = new PisXmlResponse(command, resourceType, currentFolderStr, PisUtilsResponse
				...
				if (command.equals(CommandHandler.GET_FOLDERS))
					...
					//modify by chenqizhi 2009/06/11
					/*
					...
					int errorNumber = PisXmlResponse.EN_UKNOWN;

					if (newFolder.exists())
						errorNumber = PisXmlResponse.EN_ALREADY_EXISTS;
					else {
						try {
							errorNumber = (newFolder.mkdir()) ? PisXmlResponse.EN_OK
							        : PisXmlResponse.EN_INVALID_FOLDER_NAME;
						} catch (SecurityException e) {
							errorNumber = PisXmlResponse.EN_SECURITY_ERROR;
						}
					}
...
//			String typeDirPath = getServletContext().getRealPath(typePath);
			...
			if (!currentDir.exists())
				...
				//modify by chenqizhi 2009/06/11 begin
				upload.setHeaderEncoding("UTF-8");
				//modify by chenqizhi 2009/06/11 end
				try {
					...
					//modify by chenqizhi 2009/06/11 begin
					//filename = UUIDGenerator.getInstance().generateRandomBasedUUID().toString().replace("-", "")+"."+extension;
					//modify by chenqizhi 2009/06/11 end
					if (!ExtensionsHandler.isAllowed(resourceType, extension))
						ur = new UploadResponse(UploadResponse.SC_INVALID_EXTENSION);
					else {

						filename = FileNameUtils.genFileName(extension);
		...
		if(commandStr != null && commandStr.equals("QuickUpload")){
			String msg = "<script type=\"text/javascript\">";
			msg +="window.['CKEDITOR'].tools.callFunction(" + callbackFunc+ ",'" + fileUrl + "',''" + ")";
			msg += "</script>";
			out.print(msg);
		}*/
